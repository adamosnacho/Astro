...
public class Alien extends EnemyEntity implements TipEntity, Serializable {
    ...
    private int minimumRange = 70;
    public final int dmg = 100;
    public int knockBack = 0;
    public final float maxHp;

    public final int hpBarWidth = 150;
    public final int hpBarHeight = 20;
    public final int hpBarMarginBottom = 20;

    public Alien(GamePanel gp, Player p, int x, int y) {
        height = 144;
        width = 99;
        hp = gp.terrain.randomRange(100, 200);
        maxHp = hp;
        this.gp = gp;
        this.p = p;
        this.x = x;
        ...
        try {
            sprite = ImageIO.read(getClass().getResourceAsStream("/art/png/alien.png")); // load sprite
        } catch (IOException e) {
            e.printStackTrace();
        }
        gp.enemies.add(this);
    ...
    public void draw(Graphics g) {
        float filled = hp / maxHp;
        g.setColor(new Color(255, 255, 255));
        g.fillRect((x + width / 2) - hpBarWidth / 2, y - hpBarMarginBottom - hpBarHeight, hpBarWidth, hpBarHeight);
        g.setColor(new Color(255, 0, 0));
        g.fillRect((x + width / 2) - hpBarWidth / 2, y - hpBarMarginBottom - hpBarHeight, (int) (hpBarWidth * filled), hpBarHeight);
